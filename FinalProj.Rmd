...
shuffle <- cbind.data.frame(score, preds)
shuffle <- shuffle[sample.int(nrow(shuffle),nrow(shuffle)),]
...
pred.tree2 <- predict(tree2, newdata = test)
misclas.tree2 <- sum(pred.tree2 != test$score)/length(pred.tree2)
misclas.tree2 #totally misclassified lol
...
misclas.rf1 <- sum(pred.rf1 != test$score, na.rm = T)/length(na.omit(pred.rf1))
misclas.rf1 # best one yet
```
